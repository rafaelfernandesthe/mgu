...
public class UsuarioServiceImpl implements UsuarioService {
	...
	private final List<String> mensagensValidasDynamics = Arrays.asList("Usuário criado com Sucesso",
			"Usuário ja possui cadastro. Foram adicionadas as permissões do PGMC", "Usuário ja possui CPF ou Email Cadastrado");
	...
	public Usuario salvarUsuarioMaster(Usuario usuario) throws Exception {
		...
		try {
			boolean dynamicsAtivo = parametrizacaoService.findByDcParametro(ParametrizacaoEnum.DYNAMICS_ACTIVE.getDcParametro()) != null ? Boolean
					.parseBoolean(parametrizacaoService.findByDcParametro(ParametrizacaoEnum.DYNAMICS_ACTIVE.getDcParametro())) : false;
			if (dynamicsAtivo) {
		...
		try {
			...
			e.printStackTrace();
			new MguException("MGU ALERTA: Usuário criado com sucesso, porem não foi possivel enviar a senha para o e-mail \"" + usuario.getDcEmail()
					+ "\" , favor entrar em contato com a central de serviços para solicitar a senha de acesso do usuário \""
	...
	public List<Usuario> findByPrestadorasId(Long id) {
		return usuarioRepository.findByPrestadorasId(id);
	}
