...
public class UsuarioServiceImpl implements UsuarioService {
	...
	public Usuario salvarUsuarioMaster( Usuario usuario ) throws Exception {
		...
		//
		// try {
		// ldapService.createMasterUser( usuario.getDcUsername(),
		// usuario.getDcSenha() );
		// } catch ( Exception e ) {
		// // usuarioRepository.deletarParaRollback( usuario );
		// throw new LdapException( "LDAP ERRO: Não Foi possivel criar o usuario
		// \"" + usuario.getDcUsername() + "\"" );
		// }
		//
		// try {
		// boolean dynamicsAtivo = parametrizacaoService.findByDcParametro(
		// "dynamics_active" ) != null ? Boolean.parseBoolean(
		// parametrizacaoService.findByDcParametro( "dynamics_active" ) ) :
		// false;
		// if ( dynamicsAtivo ) {
		...
		//
		// if ( StatusDynamics.SUCESSO.getValue().equals(
		// retornoDynamics.getId().getValue() ) ) {
		// if ( mensagensValidasDynamics.contains(
		// retornoDynamics.getMensagem().getValue() ) ) {
		...
		// // usuarioRepository.deletarParaRollback( usuario );
		// throw new DynamicsException( "DYNAMICS ERRO: " + transformarMensagem(
		// retornoDynamics.getMensagem().getValue() ) );
		// }
		// }
		...
		// } catch ( Exception e ) {
		// e.printStackTrace();
		// new MguException( "MGU ALERTA: Usuário criado com sucesso, porem não
		// foi possivel enviar a senha para o e-mail \"" + usuario.getDcEmail()
		// + "\" , favor entrar em contato com a central de serviços para
		// solicitar a senha de acesso do usuário \"" + usuario.getDcUsername()
		// + "\"\n\n" ).printStackTrace();
		// }
	...
	private boolean existsPerfilDynamics( Usuario usuario ) {
		// Perfil perfilDynamics =
		// perfilService.findByDcPerfilAndSistemaDcSistema( "DYNAMICS",
		// "DYNAMICS" );
